...
```{r}
LU_train$VISIT<-str_remove_all(LU_train$VISIT,"[vVisit.]")

token_train$VISIT<-str_remove_all(token_train$VISIT,"[vVisit.]")
...
```{r}
merged<-merge(demo,LU)
df_train<-merge(merged,token)
```
...
visit1<-select(visit1,SUBJ,ASD_level1=ASD_level,nonVerbalIQ1=nonVerbalIQ,VerbalIQ1=VerbalIQ,Socialization1=Socialization)
df_train<-merge(visit1,df_train)
...
filter2.7<-filter(df_train,CHI_MLU>2.7)
filter.1.5<-filter(df_train,CHI_MLU<1.5)
non_complete<-group_by(df_train,SUBJ)%>%summarize("number"=n())%>%filter(number<6)
```
...
```{r}
most6<-filter(df_train,VISIT==6)%>%arrange(desc(tokens_CHI))

least1<-filter(df_train,VISIT==1)%>%arrange(tokens_CHI)
...
```{r}
subset<-subset(df_train,Diagnosis=="ASD")%>%select(SUBJ,CHI_MLU,tokens_CHI,tokens_CHI)

#nothing happens if you include variable name twice
...
meansubjectwords<-group_by(df_train,SUBJ)%>%summarise(mean(df_train$tokens_CHI))
df_train_mean<-merge(df_train,meansubjectwords)
